...
const CONFIG = (
    ...
    block = (
        ...
        title_w    = 180,             # Width for block title label
...
mutable struct AppState
    ...
    # Default constructor initializes observables and empty buffers.
...
function start_app()
    ...
    # --- Extension slot area where the user can add custom widgets ---
    ...
    # Example callback for the extension button (prints to STDOUT / console).
    on(btn_task.clicks) do _
        println("Extension button clicked at $(state.run_time_str[])")
    end

    # Apply vertical size ratios and gap between plot rows.
                        ...
                        if length(raw_buffer) >= idx + 51
                            ...
                            if state.is_calibrating[]
                                ...
                                if length(state.calib_buffer) >= 100
                                    state.gyro_offset = sum(state.calib_buffer)/100
